...
- **2026-09-24 國家導覽重構**：國家選擇器提升為頁面最上層主導覽（跨「行程/交通查詢/資源」三個子分頁常駐），取代原本「依 country_id 分組、全部攤開」的呈現（原 MC-3 版本）；選某國後行程清單（`#trips-tab-list`）只顯示該國行程（扁平列表，不再分組），並自動選中第一個行程。
- 新增行程時選所屬國家，預設帶入目前選定的國家（`selectedCountryId`）。
- **行程的瀏覽權限與國家導覽無關**：`trips` 的 SELECT 本來就公開，任何身份（管理員、協作者、訪客）都能瀏覽所有國家的所有行程；國家導覽只是前端的呈現/篩選層，不做任何身份限制。`trip_collaborators` 的權限（`can_edit_itinerary`/`can_edit_wishlist`）只影響「能不能編輯」，不影響「看不看得到」——這點在 2026-09-24 重構驗收時曾被誤寫成「協作者只看得到自己有權的行程」，已確認為誤解並更正於此。
...
- **2026-09-24 擴充**：切換國家選擇器時，即使還沒選到某個特定子行程也會立即連動——若該國有行程，`switchCountry()` 自動選中第一個並透過既有 `selectTrip()` 觸發 `trip-changed`；若該國目前沒有任何行程，改為手動 dispatch `trip-changed`（`tripId` 為 `null`、`countryId` 有值），收藏頁的 `applyTripCountryLink()` 本來就只檢查 `countryId` 是否存在，不需要改收藏頁就能支援「選國家立即連動,不需等選到子行程」。
...
- **行程頁國家導覽重構（2026-09-24，MC 系列結案後的後續優化，非 MC 編號任務）**：MC-3 的「依國家分組、全部攤開」在國家一多時畫面過長，改為「國家選擇器提升為頁面最上層主導覽 + 選國家後只顯示該國行程」的兩層結構，詳見 3.2/3.3 節與 PROJECT_PROGRESS.md 對應章節。**驗收時的重要更正**：原始任務指令的驗收標準誤寫「協作者身份：只看得到自己有權的行程」，經確認這不符合本專案設計——`trips` 的 SELECT 本來就公開，協作者（及訪客）能瀏覽所有行程，`trip_collaborators` 權限只影響編輯，不影響瀏覽；本次重構的程式碼也未依身份過濾任何行程（`renderTripsTabList()`/`renderCountrySelector()`/`switchCountry()` 皆只依 `country_id` 篩選，未引用 `canEditItinerary`/`admin-only` 等權限判斷）。
