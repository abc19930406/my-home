...
### ✅ /trip 整合頁面（階段 1-9 全部完成，Heisenbug 案已於 2026-07-16 結案，V2 九階段全綠）
...
#### ✅ Heisenbug 案結案：OAuth 登入後 UI 間歇性未切換（2026-06-22 發現；2026-07-11 完成第一輪時序診斷；2026-07-16 結案）
...
- **失敗取證 SOP（若未來重新插入診斷碼，比照此流程）**：日常使用中任何一次登入後顯示異常，**不關閉、不重整該分頁**，開 Console 執行 `copy(sessionStorage.__auth_debug_log)` 貼出剪貼簿內容，並截圖 Console（確認有無「🔑 朋友已登入」或 ⚠️ 警告）
- **診斷設施（已移除）**：trip.astro 原插入 `[DEBUG-HEISENBUG]` 標記的 sessionStorage 時序記錄碼（commit `5724f31` 新增，寫入 `sessionStorage.__auth_debug_log`，刻意不用 console 以免改變時序），已於 2026-07-16 案件結案時整批移除（`logAuth` 函式與所有呼叫點），`onAuthStateChange` 事件流程與移除前逐行等價，未改動任何登入邏輯
- **新觀察（留待未來若重啟排查時一併評估，非本次結案範圍）**：每次視窗焦點切換，Supabase 會重發 `SIGNED_IN`，TripPlanner 的 `updateAuthUI` 隨之重查一次 `allowed_users`。功能無害，但查詢次數隨使用時間持續累積，可評估白名單查詢結果快取
- **環境凍結（2026-07-11）**：瀏覽器端 Supabase CDN 由浮動 `@2` 鎖定為確切版號 `@2.110.2`——jsdelivr 對 `@2` 會自動升 minor/patch，「SDK 版本自行變動」是 6/22 故障、7/11 無法重現的最可能解釋
- **2026-07-16 結案結論**：
  - **根因未定案**——兩嫌犯（trip.astro callback 內提前清 hash、手動 INITIAL_SESSION(null) 覆寫 SIGNED_IN）已由 2026-07-11 時序 log 排除，但未找到真正觸發條件
  - **可疑溫床已根除**——CDN 浮動版本（最可能的成因）已於 2026-07-11 鎖定確切版號，並於 V2 階段 9 核心任務（2026-07-13～07-15）進一步從 CDN 動態注入改為 npm 打包，結構性消除「SDK 版本自行變動」這個變數
  - **兩輪浸泡期均未再現**——鎖版後（2026-07-11～07-13）與 npm 遷移後（2026-07-15 起）皆持續正常使用未再出現症狀
  - **結案方式**：以「可疑溫床已移除 + 長時間未重現」作為結案依據，非「找到真正根因並修復」；診斷碼已移除，若未來再現，需重新插入 `logAuth`/`[DEBUG-HEISENBUG]` 診斷碼取證，比照上述 SOP 處理
- **狀態**：✅ 結案（非「排除嫌疑後定案根因」，而是「風險因子已除、長期未重現」的務實結案）
...
- **但書（2026-07-11 更新，2026-07-16 結案）**：2026-07-11 對照實驗已確認 DevTools 開關與登入行為無因果，上述結果可視為一般情境的有效參考；「OAuth 登入後 UI 間歇性未切換」問題已於 2026-07-16 結案（見上方「Heisenbug 案結案」），結案基礎為「可疑溫床已除、長期未重現」而非「定案根因」，階段 2.5 依此結案標記為已完成
...
### 🔶 V2 階段 9 最終任務（O3）：清空歷史遺留（已完成，僅餘 Heisenbug 兩項待浸泡期滿後獨立處理）
...
- **資料庫遺留欄位（讀寫皆由使用者於 Supabase 執行，不可逆操作慣例，已完成）**：
  ...
  - 唯讀確認 SQL 結果（`spots.category`/`spots.icon` 資料分佈）：`購物/🛍️` 1 筆、`景點/⛩️` 1 筆、`景點/🎡` 1 筆、`景點/📍`（文件記載的預設值）113 筆——絕大多數確為預設值，僅 3 筆非預設組合，但程式碼面已確認全站無任何讀寫路徑會用到這兩欄，此 3 筆歷史資料不影響任何現行功能
  - 使用者已於 Supabase SQL Editor 執行 `ALTER TABLE ... DROP COLUMN`，三欄（`spots.category`、`spots.icon`、`travel_subway_maps.trip_id`）皆一併執行成功，欄位已刪除
...
| 資料庫欄位刪除，使用者確認執行成功 | 唯讀確認 SQL + 刪除 SQL，三欄皆執行成功 | ✅ |
| Modal 命名稽核完成，範圍內修正已驗證 | 本機瀏覽器實測開關 modal | ✅ |
| V2 階段表第 9 列標記為 🔶（非全 ✅） | `PROJECT_ARCHITECTURE_V2.md` | ✅ |

---

### ✅ Heisenbug 診斷碼移除 + 案件結案（2026-07-16，O3 浸泡期滿後的獨立任務）

- **執行前確認**：使用者反映先前一輪「移除 Heisenbug 診斷碼」的任務，GitHub 上 `trip.astro` 仍有 10 處 `[DEBUG-HEISENBUG]` 標記與 `logAuth` 函式，文件也未更新。查證 `git log --all` 只找到插入診斷碼的 commit（`5724f31`）與第一輪時序診斷結論的文件 commit（`8cd1111`），**沒有任何移除診斷碼的 commit**；`git status` 確認工作區乾淨、無未提交的殘留變更。可確認的事實是：程式碼與文件都停在「診斷碼仍在線上」的狀態，前一輪任務並未在此專案留下任何 commit 痕跡（是完全沒執行，或在別的情境執行但沒有落地到這個 repo，本任務無法從這個 repo 的歷史判斷；可確定的是「未 commit/push」違反 CLAUDE.md 的部署規則，故本次重新完整執行一次）
- **移除範圍**：`src/pages/trip.astro` 的 `logAuth` 函式定義（含其 catch 區塊內的診斷註解）與全部 9 處呼叫點（page-load 基準點、`onAuthStateChange` 進入點、清 hash 前、SIGNED_IN/SIGNED_OUT 廣播前、`getSession` 返回後、手動廣播 INITIAL_SESSION 前、init 例外 catch），共 10 處 `[DEBUG-HEISENBUG]` 標記全數移除
- **邏輯完整性**：移除後 `onAuthStateChange` callback、`getSession` 初始化、`auth-state-changed` 事件廣播、`window.__latestAuthState` 快照賦值等所有實際邏輯行**逐行保留、順序未變**，只移除診斷專用的函式定義與呼叫敘述；全站搜尋 `DEBUG-HEISENBUG`/`logAuth`/`__auth_debug_log` 確認歸零
- **案件結案結論**：詳見上方「Heisenbug 案結案」章節，結案基礎為「兩嫌犯已排除 + 可疑溫床（CDN 浮動版本）已隨 npm 遷移根除 + 兩輪浸泡期均未再現」，**非**「定案根因並修復」；若未來再現需重新插入診斷碼取證
- **文件同步**：`PROJECT_PROGRESS.md`（本節 + 上方 Heisenbug 案結案章節 + 待辦清單第 1、11 項）、`PROJECT_ARCHITECTURE_V2.md`（階段 2.5、階段 9 兩列改 ✅，九、已完成項目總覽新增結案記錄）、`CLAUDE.md`（移除 CDN 過渡性註記，改寫為 npm 架構的正式硬性規則）三份文件同步更新

#### 自我驗收對照表

| 驗收項目 | 對應設計 | 結果 |
|---|---|---|
| 確認前一輪是否曾 commit/push | `git log --all`/`git status` 查證 | ✅ 確認未曾提交，本輪重新完整執行 |
| 全站搜尋不到 `DEBUG-HEISENBUG` | `grep -rn` src/ 全目錄 | ✅ |
| `onAuthStateChange` 事件流程與移除前一致 | 逐行比對移除前後程式碼 | ✅ |
| 未改動任何登入邏輯 | diff 僅刪除診斷相關行 | ✅ |
| PROJECT_PROGRESS.md 案結案 + 待辦更新 | 本輪文件同步 | ✅ |
| PROJECT_ARCHITECTURE_V2.md 階段 2.5、9 皆 ✅ | 本輪文件同步 | ✅ |
| CLAUDE.md 過渡性註記清除 | 本輪文件同步 | ✅ |
| commit + push 並貼出終端機輸出 | 見下方 | ✅ |
| 使用者登入實測（DevTools 關閉） | 等 Vercel Ready 後由使用者實測 | ⏳ 待使用者測試 |
...
### ✅ 全站 Pull to Refresh 誤觸修復：Modal 內捲動下拉時停用（2026-08-06，已結案）

`/notes` 開發期間發現：Modal 內容捲到底繼續往下拉時，會被 `Layout.astro` 的下拉刷新腳本誤判成「頁面頂端下拉」，整頁重新整理。這是全站共用腳本的既有 bug（任何有長 Modal 的頁面都可能中招），不是 `/notes` 專屬。

- **繞了一次彎路**：第一次診斷誤判為「內容短的頁面稍微往上滑也會誤觸」，改了 `touchmove` 判斷邏輯 + 加 `overscroll-behavior-y: contain`（commit `3b5cf39`）。使用者實機指出真正根因是「Modal 內捲動到底繼續下拉」，且新加的 `overscroll-behavior` 反而關掉了原生下拉刷新手感，於是完整還原該次改動（commit `22b97be`，`git diff` 確認與改動前逐字元相同）
- **正確根因**：下拉刷新腳本判斷整個 `window.scrollY`，無法區分「頁面頂端下拉」與「Modal 內部捲動到底後繼續下拉」
- **正確修法**（commit `35f09a4`）：`touchstart` 開頭加一個閘門，`body` 有 `modal-open` class（全站 Modal 開啟時的既有慣例）時直接放棄追蹤，不設 `isPulling`；`scrollY`/`deltaY` 既有判斷邏輯完全不動，不使用 `overscroll-behavior`
- **已知落差，本次不處理**：`admin.astro`（原生 `<dialog>`）與 `ledger.astro`（三個 Modal 皆只切換 `display`）未設 `body.modal-open`，這次的閘門對它們無效，經使用者確認暫不處理，留待未來獨立任務

#### 自我驗收對照表

| 驗收項目 | 對應設計 | 結果 |
|---|---|---|
| a. `/notes` 開 Modal、內部捲到底、往下拉不再誤觸重整 | 使用者實機測試 | ✅ |
| b. `/notes` 無 Modal、頁面頂端下拉正常觸發重整 | 使用者實機測試 | ✅ |
| c. 首頁等其他頁面下拉刷新正常 | 使用者實機測試 | ✅ |
| d. 其他有 Modal 的頁面（短文/收藏/行程）開 Modal 下拉不誤觸 | 使用者實機測試 | ✅ |
| commit + push 並貼出終端機輸出 | `3b5cf39`（誤判，已還原）→ `22b97be`（還原）→ `35f09a4`（正確修法） | ✅ |
...
- **追加修正（使用者實測時發現，同一根因的延伸案例）**：`posts/index.astro` 的 `refreshPostsList()` 在沒有 session 時提前 `return`，導致負責更新登入/登出按鈕的 `checkAdmin()`（透過 `bindActions()` 呼叫）完全沒機會執行，登出後兩顆按鈕卡在預設的 `display:none`，看起來像沒有登入入口。修法：把 `checkAdmin()` 呼叫移到提前 return 之前，讓按鈕顯示狀態不受「有無 session」擋住（commit `069aa37`）
- **同一模式的第二個案例**：使用者依「排查其他頁面」驗收時，發現 `quotes/index.astro` 的 `refreshQuotesList()` 有一模一樣的提前 return 問題，登出後語錄頁同樣不顯示登入按鈕。比照同樣手法修正（commit `8d663f2`）
...
| c. 排查修正的其他頁面登入狀態正常 | 發現 `quotes` 同一根因的登出後無登入按鈕，修正（`8d663f2`）後使用者確認通過 | ✅ |
| b、d（管理功能、桌機） | 使用者確認「手機與桌機皆已修正完成」 | ✅ |
| e. 短文三級 visibility 顯示正確 | 使用者確認通過 | ✅ |
...
### ✅ MC-5b：國家協作前端 + 探索功能改為日本專屬（2026-09-17，已結案）

MC-5a 權限地基之上，在 `JapanCollection.astro` 實作對應前端，設計依據 `PROJECT_NOTES_MULTICOUNTRY.md` 附錄 MC-5 E 節。

- **國家協作管理 UI**（僅管理員）：國家管理清單每列新增 👥 按鈕，開啟獨立 Modal 管理該國授權朋友（email 新增/移除），比照 `TripPlanner.astro` 的 `trip_collaborators` 管理模式，但國家層級只有單一權限（能不能在此國新增品項）
- **被授權朋友的新增/自管 UI**：「新增品項」入口從純 `admin-only` 拆出獨立顯示邏輯，管理員或目前切換中國家的被授權協作者皆可見，切換國家時重新評估；表單「國家」下拉對非管理員只列出自己被授權的國家；卡片編輯/刪除鈕改依 `created_by` 判斷（`canEditOwnItem`），管理員恆可，其餘僅能動自己建立的品項，點擊處理常式也重新檢查一次防止 DOM 被竄改繞過；新增品項一律帶 `created_by`（含管理員）
- **願望清單放行**：`canWishlistItem()` 新增國家協作者分支（不分是否歸屬行程）；登入守門邏輯放寬為「在白名單 OR 是任一國家協作者」皆可進入，避免純國家協作者被既有的「僅限受邀用戶」檢查提前踢出
- **⚠️ 開工前發現並與使用者確認的問題**：附錄列的 5 個文字動態化位置中，有 4 個（探索標題、搜尋中提示、結果標題、快速收藏成功訊息）其實都屬於「探索日本」功能，其後端 `/api/explore.ts` 本身寫死搜尋日本（query 固定加「日本」、`gl=jp`），若只動前端標籤會讓訊息不實。使用者決定維持這 4 處寫死「日本」，只動了刪除預設國家保護提示這一處真正國家無關的文字
- **⚠️ 追加調整（使用者實測發現，2026-09-17）**：探索區塊本身沒有跟著國家切換隱藏，導致切到泰國仍看到「探索日本」——原規劃的「文字動態化」方向本來就沒涵蓋「整塊隱藏」，是設計遺漏。使用者決定調整方向：探索功能維持日本專屬，不做多國適配，改為讓整個探索區塊只在目前切換中的國家 `is_default`（日本）時顯示，切到其他國家整塊隱藏（判斷依據用 `is_default` 而非國家名稱字串，避免改名後失效）；「加入收藏」的成功訊息因為是全國家通用功能，從寫死「日本」改為中性文字「已成功加入收藏」
- **順手修正的兩個既有缺陷**：`japan_items` 的 UPDATE/DELETE 原本沒有 `.select()` 檢查受影響筆數，RLS 擋下時會誤報成功——MC-5b 起非管理員的寫入才會真的被 RLS 擋，此坑必須先補；編輯品項原本從卡片 dataset 重組資料，遺漏 `created_by`，改用 `itemsCache` 完整物件（一併修正了「編輯時歸屬行程未帶入」的既有缺陷，MC-2 已修正過一次類似問題，這次是同一手法補上新欄位）
- **明確排除本階段**：不動 RLS（MC-5a 已完成）、不改命名、不做探索功能的多國適配、不動 `trip_collaborators` UI

#### 自我驗收對照表

| 驗收項目 | 對應設計 | 結果 |
|---|---|---|
| a. 管理員對某國新增/移除/再加授權朋友，清單正確 | 使用者實測確認 | ✅ |
| b. 被授權朋友：授權國家顯示新增入口並可新增（created_by 為自己）；非授權國家無入口 | 使用者實測確認 | ✅ |
| c. 被授權朋友：只能編輯/刪除自己建立的品項；可標願望與填數量 | 使用者實測確認 | ✅ |
| d. 未授權朋友：任何國家都無新增入口，行為與之前一致 | 使用者實測確認 | ✅ |
| e（初版）. 文字動態化無殘留「日本」 | 使用者實測發現探索區未隨國家隱藏，追加調整（見上方備註） | ✅（追加調整後） |
| f. 管理員全功能正常；手機桌機、Modal 正常 | 使用者實測確認 | ✅ |
| 追加 a. 目前國家=日本：探索區正常顯示與運作 | 使用者實測確認 | ✅ |
| 追加 b. 切到韓國/泰國：探索區完全不見，無殘留「日本」 | 使用者實測確認 | ✅ |
| 追加 c. 各國新增收藏成功提示不顯示錯誤國名 | 使用者實測確認 | ✅ |
| 追加 d. 切回日本：探索區重新出現 | 使用者實測確認 | ✅ |
| 追加 e. 手機與桌機正常 | 使用者實測確認 | ✅ |
| commit + push 並貼出終端機輸出 | `00200a5`（協作前端）、`b746ce8`（探索區調整） | ✅ |

#### 事後修正：協作者新增品項時「主分類」下拉為空（2026-09-17，`f4a5e31`，已驗收）

MC-5b 驗收後使用者發現：被授權協作者打開「新增品項」Modal，主分類下拉只剩「請選擇主分類」，選不到任何分類。MC-5b 驗收表當時沒有把「協作者能否選分類」列為獨立驗收項，是驗收覆蓋的缺口，不是先前測過又壞掉。

- **根因**（先驗證再修）：不在權限，也不在 `categoriesCache`。`init()` 一開始就無條件抓取 `japan_categories`（該表 SELECT 對所有人開放），管理員與協作者拿到同一份資料。真正的問題是 `populateMainCategoryDropdown()`（把 `categoriesCache` 寫進 `#form-main-cat` 的函式）只在 `init()` 的 `isAdminUser` 分支被呼叫一次——MC-5b 之前只有管理員能開這個 Modal，這樣寫沒問題；MC-5b 讓協作者也能開同一個 Modal 後，沒有人補上讓協作者觸發這次填充，下拉因此永遠停在靜態的預設選項
- **修法**：`openModal()` 開頭統一呼叫 `populateMainCategoryDropdown()`，新增與編輯皆適用、不分身分、每次開啟重新填充，不再依賴「管理員登入時填一次、之後靠 DOM 殘留選項」。該函式是純函式，只讀 `categoriesCache`，不含權限判斷
- **權限邊界不變**：不動 RLS、不放寬 `japan_categories` 任何權限；分類的新增/編輯/刪除（管理面板、新增主分類鈕）仍維持 admin-only，協作者只能「選用」既有分類
- **教訓**：某個功能從「僅管理員」開放給其他角色時，要逐一檢查它依賴的初始化步驟是否只掛在管理員分支上（下拉填充、快取、事件綁定），這類步驟在原本只有管理員使用時不會出問題，開放後才會暴露

| 驗收項目 | 結果 |
|---|---|
| a. 協作者開新增品項 Modal：主分類下拉顯示所有分類、可選；子分類正常連動 | ✅ 使用者實測確認 |
| b. 協作者選好分類儲存：品項帶正確分類、`created_by` 為自己 | ✅ 使用者實測確認 |
| c. 協作者仍看不到新增主分類/管理分類入口（只能選、不能管理分類） | ✅ 使用者實測確認 |
| d. 管理員本人分類下拉與分類管理一切如常 | ✅ 使用者實測確認 |
| e. 手機與桌機、Modal 正常 | ✅ 使用者實測確認 |
...
### ✅ MC-4：行程→收藏的單向國家連動（2026-09-17，已結案，多國家擴充 MC-1～MC-5 全部完成）

依 `PROJECT_NOTES_MULTICOUNTRY.md` 3.3 節，實作切換行程時收藏頁自動切到該行程所屬國家。至此多國家擴充系列全部結案。

- **`TripPlanner.astro`**：`selectTrip()` 發送的 `trip-changed` 事件 payload 擴充帶上該行程的 `country_id`，直接從 `tripsCache` 取得，不需額外查詢
- **`JapanCollection.astro`**：新增 `applyTripCountryLink(tripId, countryId)`，監聽 `trip-changed` 後自動切換國家切換器並套用該行程的收藏篩選（沿用既有 V2 階段 3 的行程篩選邏輯，比對 id）；監聽器提前綁定（在任何 `await` 之前），比照本專案既有的 Race Condition 防護慣例
- **邊界處理**：`countryId` 為空、或在收藏頁的 `countriesCache` 內找不到對應國家時，維持目前國家不動，不報錯不清空
- **單向連動天然成立**：收藏頁的國家切換器點擊處理常式完全沒有改動，手動切換國家不會發送任何事件給行程頁，不需要額外的鎖或旗標防止反向連動；連動後使用者若再手動切換國家，直接覆蓋 `selectedCountryId`，manual 天然覆蓋 auto
- **驗證方式**：先用構造的 CustomEvent 模擬各種情境（切到已知的泰國 id、不存在的 id）確認核心邏輯無誤且不報錯；接著用使用者測試 MC-3 時留下的真實泰國行程「2026泰國之旅」做端到端實測——非僅模擬，是真實資料走完整流程
- **明確排除**：不做收藏→行程反向連動、不動 RLS、不改命名

#### 自我驗收對照表

| 驗收項目 | 對應設計 | 結果 |
|---|---|---|
| a. 切到日本行程，收藏頁國家停在日本、套用該行程篩選 | 使用者實測確認 | ✅ |
| b. 切到韓國/泰國行程，收藏頁自動切到對應國家、套用篩選 | 使用者實測確認（真實泰國行程「2026泰國之旅」端到端測試） | ✅ |
| c. 連動後手動切到別的國家：以手動為準 | 使用者實測確認 | ✅ |
| d. 收藏頁手動切國家，行程頁不受影響 | 使用者實測確認 | ✅ |
| e. 只有日本時連動正常無異常 | 使用者實測確認 | ✅ |
| f. 手機桌機正常 | 使用者實測確認 | ✅ |
| commit + push 並貼出終端機輸出 | `9148980` | ✅ |

---

### ✅ 收藏頁品項備註展開/收合（2026-09-19，已結案）

`JapanCollection.astro` + `japan.css`：備註過長時預設折疊，避免長備註撐長版面。純前端呈現調整，不動資料、RLS、命名。

- **折疊判斷獨立成 `noteNeedsCollapse()`**：依實際顯示行數（內容高度 ÷ 行高）判斷，不看字數（中英文與換行的字數差異大，行數才準）；門檻常數 `NOTE_COLLAPSE_LINES = 7`，同步寫入 CSS 變數 `--note-max-lines`，判斷與樣式共用同一個值。使用者日後可能改為「點擊彈出 Modal 顯示全文」，屆時只需替換 `handleNoteToggle()`，判斷不必動
- **行為**：超過 7 行才折疊（`line-clamp` 7 行 + 「⋯展開」）；7 行以內整段顯示、不出現任何按鈕；展開後變「收合」
- **原地切換、不跳位**：只切換 class，不重繪清單。展開時捲動位置與卡片頂端不動；收合時內容變短會讓按鈕上移，故補償捲動量，讓按鈕停在視窗中同一位置（本機驗證按鈕位移 0，卡片仍在視線內，不跳回頂端）
- **展開狀態記在 `expandedNoteIds`**（以品項 id）：整批重繪（`init()`／篩選）後不會被折回
- **重判時機**：收藏分頁初始是隱藏的（量不到行數）、視窗寬度會變（手機轉向），故掛在 `ResizeObserver`／`resize`／`document.fonts.ready`，以 `requestAnimationFrame` 合併觸發；量不到時回傳 `null`，維持原狀不誤判
- **不影響其他卡片操作**：展開鈕的點擊在既有卡片點擊委派最前面處理後直接 `return`，編輯/刪除/願望清單等路徑未動
- **明確排除**：不做 Modal 彈窗（保留為日後選項）、不動資料與 RLS、不改其他卡片元素

#### 自我驗收對照表

| 驗收項目 | 對應設計 | 結果 |
|---|---|---|
| a. 超過 7 行：預設 7 行 + 展開鈕；展開/收合視線停在該卡片，不跳位 | 使用者實測確認；本機另以真實資料驗證（桌機 96 則備註中 6 則超過，皆截為 7 行有鈕） | ✅ |
| b. 7 行以內：整段顯示、無展開鈕 | 使用者實測確認 | ✅ |
| c. 展開收合不影響編輯/刪除/標記 | 使用者實測確認 | ✅ |
| d. 手機與桌機皆正常 | 使用者實測確認；本機手機寬度 16 則超過，判斷零誤差，切回桌機自動重判為 6 則 | ✅ |
| e. 大量品項清單中段的卡片展開再收合，不跳回頂端 | 使用者實測確認；本機量測捲動位置未歸零 | ✅ |
| commit + push 並貼出終端機輸出 | `cee5f64` | ✅ |
...
| d. XSS 注入（`<script>`/屬性跳脫）顯示為純文字、不執行 | 本機標準化測試（合成惡意字串）驗證；正式站已由協作者帳號對 name/address/open_hours/price/note 五欄位實測 payload，全數顯示為純文字、無彈窗、搜尋功能正常 | ✅ |
...
### ✅ 行程頁（/trip）國家導覽重構：國家選擇器提升為最上層主導覽（2026-09-24，已結案）

MC-3（2026-09-17）的「行程清單依國家分組、全部攤開」在國家一多時畫面會變得很長，改為「先選國家 → 再選子行程」兩層結構，commit `5f5776a`：

- **國家選擇器移到頁面最上層**：新增 `#trip-countries-tab-list`（`.trip-country-tab`，class 名獨立於 `JapanCollection.astro` 既有的 `.country-tab`，避免兩元件同頁共存時 CSS 互相污染），位置在 `<section class="trip-subtab-bar">`（行程/交通查詢/資源）之前，跨三個子分頁常駐顯示，不隨子分頁切換而消失
- **`#trips-tab-list` 簡化為扁平列表**：拿掉 MC-3 的 `groupTripsByCountry()`/`.trip-group`/`.trip-group-header` 分組結構，只顯示目前選定國家（`selectedCountryId`）的行程，不留兩套並存的分組邏輯
- **`switchCountry(countryId)`**：切換國家後重繪國家選擇器與行程清單，並自動選中該國第一個行程（透過既有 `selectTrip()`，與手動點行程標籤走完全相同的路徑，不新增額外的地圖 `fitBounds`）；該國沒有任何行程時顯示空狀態（新抽出的 `showEmptyTripState()`，與既有「刪除最後一筆行程」的空狀態共用），並手動 `dispatch` 一次 `trip-changed`（`countryId` 有值、`tripId` 為 `null`），讓收藏頁在還沒選到具體行程前就能立即連動切換國家——不需修改 `JapanCollection.astro`，它既有的 `applyTripCountryLink()` 本來就只檢查 `countryId` 是否存在
- **新增/編輯/刪除行程配合調整**：存檔後若行程所屬國家與目前選定國家不同，先切換國家選擇器再重繪，確保存檔後看得到剛存的行程；刪除行程後優先選「目前選定國家內」剩下的行程，不再像重構前直接跳去全域第一筆（可能是別國的行程）；新增行程 Modal 的國家預設值改為目前選定的國家

**明確排除**：未動任何 RLS 與協作權限；未改命名；未動收藏頁內部邏輯（只靠既有 `trip-changed` 事件互動）；未改變選到行程「之後」的任何既有功能（地圖、每日行程、交通查詢、資源子分頁行為不變）。

**驗收標準更正（重要）**：原始任務指令的 f 項驗收標準誤寫「協作者身份：只看得到自己有權的行程」，經使用者於驗收後指出並確認——這不符合本專案設計。`trips` 表的 SELECT 政策本來就公開，管理員、協作者、訪客皆能瀏覽所有國家的所有行程；`trip_collaborators`（`can_edit_itinerary`/`can_edit_wishlist`）只影響「能不能編輯」，從未影響「看不看得到」。複查本次重構程式碼（`renderTripsTabList()`/`renderCountrySelector()`/`switchCountry()`）確認皆只依 `country_id` 篩選，未引用任何身份或編輯權限判斷，沒有依身份隱藏任何行程。已同步更正 `PROJECT_NOTES_MULTICOUNTRY.md` 3.2 節的對應描述。

**驗證方式**：`astro check` 修改後 1491 則錯誤（基準 1493，反而更少——新函式補上型別標註，舊的分組函式一併移除，非新增缺陷）；本機 dev server 實測日本/泰國切換、地圖 `fitBounds` 行為、收藏頁即時連動、交通查詢子分頁下國家選擇器持續可見、手機版（375px）版面無重疊。

#### 自我驗收對照表

| 驗收項目 | 對應設計 | 結果 |
|---|---|---|
| a. 最上層有國家選擇器，選某國後顯示該國子行程並預設展開第一個，不再一次攤開所有國家 | 使用者實測確認 | ✅ |
| b. 切換國家：子行程清單隨之更換；收藏頁立刻連動到該國 | 使用者實測確認；本機 dev server 亦驗證 | ✅ |
| c. 選子行程：地圖、每日行程正常載入，與重構前一致；收藏套用該行程篩選 | 使用者實測確認 | ✅ |
| d. 交通查詢、資源子分頁在選定行程後正常 | 使用者實測確認 | ✅ |
| e. 只有日本一國時，結構正常不突兀 | 使用者實測確認（本機測試環境為日本+泰國兩國，無法本機驗證單一國家情境） | ✅ |
| f. 協作者身份：與管理員、訪客一樣能瀏覽所有國家與所有行程，協作者權限只影響能否編輯，不影響能否瀏覽；國家導覽正常 | 使用者實測確認；程式碼複查確認未依身份過濾行程（見上方「驗收標準更正」） | ✅ |
| g. 手機與桌機、Modal、地圖視野（切換不重置）皆正常 | 使用者實測確認；本機 375px 視窗亦驗證版面無重疊 | ✅ |
| commit + push 並貼出終端機輸出 | `5f5776a` | ✅ |
| 文件同步（本節 + PROJECT_NOTES_MULTICOUNTRY.md 3.2/3.3 節） | 本次一併完成 | ✅ |
...
### ✅ 首頁標題編輯、站名全站連動、靜態頁重讀與管理頁編輯入口（2026-09-25，已結案）

同一條需求線的一批 commit：`dfd6012`（首頁標題編輯）、`a4b5a05`（/admin 標題接資料庫）、`91748e6`（站名連動與分頁標題）、`ee10294`（卡片載入後重讀）、`14d4537`（/admin 編輯入口與共用邏輯）。

- **首頁標題/副標題管理員線上編輯（`dfd6012`）**：沿用「編輯狀態」機制，資料存 `status` 表 id=1，新增可空欄位 `site_title`/`site_subtitle`（站名唯一資料來源，空值回退預設，不空白）。前置 SQL 由使用者執行並單獨確認完成
- **盤點發現並修復的安全缺口**：`status` 原 UPDATE 政策是「任何登入帳號可改」（`SECURITY_AUDIT.md` 早已列為 LOW 未修復），若標題存進去，朋友帳號能從 Console 改站名；同時首頁編輯入口只判斷有無 session，朋友登入也看得到「編輯狀態」按鈕。修復：`status` UPDATE 政策改 `is_admin()`（SELECT 公開不動）、前端入口改比對 `PUBLIC_ADMIN_EMAIL`（同時修正既有「編輯狀態」按鈕的問題，屬既有行為收緊）。使用者實測非管理員 Console 寫入被拒
- **`/admin` 標題不同步（`a4b5a05`）**：管理員驗收 a 回報「訪客看到新標題、管理員仍是舊的」。根因：管理員登入後預設導向 `/admin`，該頁標題與副標題是寫死的，並非首頁存檔失敗
- **站名全站連動（`91748e6`）**：全站盤點站名出現位置後決定：首頁/管理頁頁尾版權連動；`Layout.astro` 原本**從未接收 `title` 參數**（`<title>` 永遠寫死「The Corner Table」，各頁傳入的「記帳系統 | The Corner Table」等從未生效，所有分頁標題一模一樣），修為「頁面名 | 站名」，五個子頁（trip/ledger/quotes/posts/notes）改為只傳頁面名；新增 `src/lib/site-title.ts`（`getSiteTitle()`，60 秒記憶）。**維持寫死**（同意判斷）：網頁描述、iOS 主畫面名、PWA manifest、AI 系統提示。Polaroid 頁原傳「Polaroid | 一天一句」，現顯示為兩個分隔線，不在指定範圍未動
- **靜態頁改完不即時生效（`ee10294`）**：首頁與 `/admin` 為 prerender，cards 烤進 HTML。新增 `src/lib/live-cards.ts`，載入後重讀 `cards` 表**原地更新**既有卡片（不整區重繪，保留 scoped CSS 與旅行地圖卡互動）；讀取失敗/空結果/分區卡片數不一致一律保留靜態版。順帶發現 `/admin` 卡片來源原為本機 `links.ts`（重新整理永遠舊內容，Modal 以標題比對更新，舊標題會匹配 0 列卻不報錯），改為與首頁一致（`cards` 表，失敗回退 `links.ts`）；卡片編輯 Modal 未動
- **`/admin` 編輯入口（`14d4537`）**：把首頁標題編輯邏輯抽成 `src/lib/title-editor.ts`，首頁與管理頁共用同一份，讀寫同一份 `status`；「 (管理模式)」是顯示層後綴，編輯框與寫入的永遠是純副標題值
- **已知限制**：頁面為靜態，標題編輯與分頁標題（`<title>`）要等重新部署才進靜態版；標題/頁尾/卡片以載入時重讀補償，分頁標題無補償
- **相關缺口已於 2026-09-26 補上**：`cards` 表寫入權限同樣為「任何登入帳號可改」，本批只收緊 `status`；`cards` 於 2026-09-26 比照修復（UPDATE 改 `is_admin()`，全專案唯一寫入點為管理頁卡片編輯 Modal，INSERT/DELETE 本無政策），並補上 Modal 的寫入筆數檢查（`86900aa`，被 RLS 擋下時不再誤報「已儲存」）。使用者實測管理員編輯正常、朋友帳號 Console 寫入被拒、訪客首頁正常，詳見 `SECURITY_AUDIT.md`「cards」小節
- **`/admin` 進入權限收緊（2026-09-26，`4e4c2b0`）**：原本只檢查有無 session，朋友/家人帳號登入後可打開 `/admin` 看到卡片編輯等管理介面（資料庫寫入已擋，但不該讓非管理員看到後台），與先前修過的 `ledger.astro` 同類。比照 `ledger.astro` 做法：已登入但 email 不等於 `PUBLIC_ADMIN_EMAIL`（不分大小寫；未設定一律拒絕）→ alert「僅管理員可使用管理頁面」並導回首頁（非導去登入頁）；未登入維持導向 `/login`；頁面內容預設 `display:none`，通過檢查才顯示。使用者實測管理員正常進入、朋友帳號被導回首頁並提示、未登入導向不變、手機/桌機正常（a-d 全數通過）
- **驗證**：每個 commit 皆 `astro check`（錯誤數不增；共用模組化後由 1562 降至 1537）與 `astro build` 通過，本機以模擬資料驗證核心邏輯

#### 自我驗收對照表

| 驗收項目 | 結果 |
|---|---|
| 首頁標題編輯：管理員存檔生效、重整保持、訪客無入口且 Console 寫入被拒、無資料回退預設、頁尾/分頁標題不受影響 | ✅ 使用者實測（a-e；a 因 `/admin` 硬編碼問題另修） |
| `/admin` 標題同步 | ✅ 使用者確認 |
| `/admin` 編輯入口：兩頁互相同步、副標題後綴不被存入、讀取失敗回退預設、非管理員進不了 /admin、手機/桌機 | ✅ 使用者實測（a-f） |
| 站名連動（頁尾、五個子頁分頁標題） | ✅ 使用者確認正常（2026-09-26） |
| 卡片載入後重讀 | ✅ 使用者確認正常（2026-09-26） |
| commit + push | ✅ `dfd6012`、`a4b5a05`、`91748e6`、`ee10294`、`14d4537` |
| 文件同步（PROJECT_PROGRESS.md、PROJECT_ARCHITECTURE.md、SECURITY_AUDIT.md） | ✅ 本次完成 |
...
### ~~多國家擴充~~ ✅ 2026-09-17 全部完成（MC-1～MC-5，詳見 PROJECT_NOTES_MULTICOUNTRY.md）
...
4. ~~**MC-4：行程→收藏的國家連動（trip-changed 事件帶 country_id）**~~ **✅ 2026-09-17 已完成**，詳見上方「MC-4」章節
...
| quotes | 語錄收藏 | ✅ |
| status | 現在狀態（id=1 固定；含 site_title、site_subtitle 站名來源） | ✅（2026-09-25 新增站名欄位、UPDATE 收緊為僅管理員） |
...
> 2026-09-24 全面重新盤點：舊清單自 2026-07-16（V2 階段 9 結案）後未再更新，其間完成的 V3-1/V3-2、多國家擴充 MC-1～MC-5、行程頁國家導覽重構等皆未反映於此。這次以「目前真正尚未完成」為準重寫；歷史沿革與已結案項目請查閱上方「一、已完成功能」對應章節，不在此重複列出。

1. **ESC 鍵/Modal 背景點擊關閉的手動驗證**（commit `2317d2d`，2026-06-22 遺留至今）：`TripPlanner.astro` 的 `wishlist-modal`/`spot-detail-modal` 補上 ESC 關閉呼叫後，程式碼經肉眼核對邏輯正確、已部署，但從未取得使用者手動測試 ESC 鍵、背景點擊是否都能正確關閉並清除 `body.modal-open` 的明確結果（原定驗證因發現其他問題而中斷）
2. **`body.trip-page` class 缺漏評估**（V2 階段 7 發現，2026-07-16 標記「維持待評估」，詳見 PROJECT_ARCHITECTURE_V2.md 5.3 節）：`Layout.astro` 從未把 `body.trip-page` 加到 `<body>`，導致 `.trip-main-content` 缺少 `height:100vh`+`overflow:hidden` 版面鎖定，AI 對話框呈現「隨內容增高、捲動整個分頁」而非「固定視窗高度」；功能不受影響，是否修正待評估（影響全站 /trip，非僅 AI 分頁）
3. **程式碼清理階段**（獨立規劃，待全部功能穩定後執行）：統一 Modal 開關/CSS class 命名規則、移除殘留冗餘邏輯
4. **V3-3：讀書筆記本體 CRUD**（筆記新增/編輯/刪除、標籤、圖片上傳）：V3-1（資料地基，2026-08-06）、V3-2（出處/主題管理，2026-08-06）已完成，V3-3 尚未開始，模組目前暫停中
5. ~~**`cards` 表寫入權限收緊**~~ **✅ 2026-09-26 已完成**：UPDATE 政策由「任何登入帳號」改為 `is_admin()`（INSERT/DELETE 本無政策），管理頁 Modal 補寫入筆數檢查（`86900aa`），詳見 `SECURITY_AUDIT.md`「cards」小節

未來構想（食記、年度回顧、作品集、書籤收藏、習慣打卡、旅行地圖每日行程升級為 Google Maps Directions API 真實路線）記錄於下方「二、規劃中功能」的「其他規劃中功能」，尚未排入優先順序，不列入此清單。
