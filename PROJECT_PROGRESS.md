...
- **⚠️ 未處理的相關缺口**：`cards` 表的寫入權限（`SECURITY_AUDIT.md` 矩陣）仍為「任何登入帳號可改」，本批只收緊 `status`，`cards` 未動（已列入第六節待辦）
...
| 站名連動（頁尾、五個子頁分頁標題）與卡片載入後重讀 | ⚠️ 已部署，使用者未逐項回報驗收結果（後續任務接續進行），此欄待補確認 |
...
5. **`cards` 表寫入權限收緊**（2026-09-25 標題編輯盤點時確認）：`SECURITY_AUDIT.md` 矩陣列為 LOW，UPDATE 仍是「任何登入帳號可改」，`status` 已比照修為 `is_admin()`，`cards` 尚未處理；需先盤點寫入方（`admin.astro` 卡片編輯 Modal）與現行政策，再依「先查後改」流程處理
