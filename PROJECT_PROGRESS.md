...
| a.「幫我把清水寺加進 Day 1」→ 景點真實出現在行程，AI 回覆明確描述；行程分頁重整後可見 | `add_spot`（含 day_number/position） | ❌ 第一次實測失敗→已修復，待重新實測 |
...
| e. 對照 a-d 每項，資料庫實際狀態與 AI 宣稱一致（重整頁面驗證，不信對話文字） | 全部工具的 0 筆受影響檢查 | ✅（b/c/d 已確認）,a 待補測 |
