...
1. ~~「OAuth 登入後 UI 間歇性未切換」Heisenbug~~ **✅ 2026-07-16 結案**（見上方「Heisenbug 案結案」）：兩嫌犯已排除，可疑溫床（CDN 浮動版本）已除，鎖版後與 npm 遷移後兩輪浸泡期均未再現，診斷碼已移除；結案基礎為「風險因子已除、長期未重現」，非「定案根因」，若未來再現需重新取證。2026-07-11 驗證任務 D 時新增的疑似同根因家族觀察（朋友帳號登入 `/trip` 一度被誤判成管理員）維持未確認狀態，非本次結案範圍
...
11. ~~**階段 9 最終任務（O3）**：程式碼清理與重構，含 CSS/ID 前綴隔離補齊、`spots.category`/`spots.icon`/`travel_subway_maps.trip_id` 遺留欄位、根目錄除錯遺留、退役檔案~~ **✅ 2026-07-16 全部完成**，詳見上方「V2 階段 9 最終任務（O3）」；Heisenbug 診斷碼移除與案件結案記錄已於同日以獨立任務完成（見上方「Heisenbug 案結案」，V2 九階段全數 ✅）。**`body.trip-page` class 評估（V2 階段 7 發現的既有落差，見 PROJECT_ARCHITECTURE_V2.md 5.3 節）不在本次任務範圍，維持待評估**
