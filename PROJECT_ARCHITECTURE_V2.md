...
- **收藏** 🔶：日本收藏品瀏覽、願望清單、數量調整已上線；依行程篩選規劃中
...
- ✅ `window.currentTripId` 暴露機制（selectTrip() 設定 + dispatchEvent('trip-changed')），供未來篩選邏輯讀取
...
- 📋 **依行程篩選**（尚未開發）：
  ...
  - 確認既有資料 `trip_id` 為 NULL 時不受影響，現有願望清單/白名單機制皆保留不變
...
| 3 | 收藏分頁整合：japan_items.trip_id、收藏分頁顯示邏輯、一般收藏 vs 行程收藏 | 📋 待開始 |
