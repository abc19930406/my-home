...
- **編輯任何 `PROJECT_*.md` 或 `SECURITY_AUDIT.md` 時,一律採「讀取現有全文 → 在其上局部新增/修改 → 寫回」的方式,嚴禁整份覆寫(重新產生整個檔案再蓋掉)**:每次文件變更的 commit,都必須在 commit 訊息或回報中明確說明「新增/修改了哪一段」,不得只寫「更新文件」。原因:本專案曾兩次發生文件被舊版覆寫、進度標記無聲消失的事故,文件是全專案的事實來源,一旦脫節會誤導後續所有工作
